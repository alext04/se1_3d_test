...
public class AppResource extends BaseResource {
    ...
    public Response log(
        ...
        LogCriteria logCriteria = getLogCriteria(level, tag, message);

        MemoryAppender memoryAppender = getAppender(APPENDER_NAME);
    ...
    public Response mapPort() throws JSONException {
        ...
        JSONObject response = new JSONObject();
        if (!mapPort(request.getServerPort())) {
    ...
    private MemoryAppender getAppender(String appenderName) {
        Logger logger = Logger.getRootLogger();
        Appender appender = logger.getAppender(appenderName);

        if (appender == null || !(appender instanceof MemoryAppender)) {
            throw new ServerException("ServerError", appenderName + " appender not configured");
        }

        return (MemoryAppender) appender;
    }

    private boolean mapPort(int port) {
        return NetworkUtil.mapTcpPort(port);
    }

    private LogCriteria getLogCriteria(String level, String tag, String message) {
